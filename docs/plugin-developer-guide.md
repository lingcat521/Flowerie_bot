...
>
> ⚠️ **谁决定消息边界？** 这里是**插件自己决定**（你传什么就发什么）。与之相对，
> AI 侧的 **Native Reply Tool** 由**模型自主决定**拆不拆、拆几条（同一套 `MULTI_REPLY_*` 限制、
> 同一条发送链路，见 [configuration.md](configuration.md#ai-自主拆分native-reply-tool同一开关)）。
