...
| `friend_request` / `group_join_request` / `group_invited_join_request` | `request` | `request_kind` = friend/group（字段级映射**已完成**：`request_scene`/`request_id`/`request_uid`/`request_filtered`/`comment`）|
