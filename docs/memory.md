...
- 键位速查（全部 18 个，均可在「配置 → 花语记忆」改）：

| 类别 | 键 |
| :--- | :--- |
| 总开关 | `BLOSSOM_MEMORY_ENABLED` |
| 子开关 | `BLOSSOM_MEMORY_EMBEDDING_ENABLED` / `BLOSSOM_MEMORY_RERANKER_ENABLED` / `BLOSSOM_MEMORY_EXTRACT_ENABLED` / `BLOSSOM_MEMORY_RETRIEVAL_ENABLED` |
| 向量链路 | `BLOSSOM_MEMORY_EMBEDDING_MODEL` / `BLOSSOM_MEMORY_EMBEDDING_API_URL` / `BLOSSOM_MEMORY_EMBEDDING_API_KEY` |
| 重排链路 | `BLOSSOM_MEMORY_RERANKER_MODEL` / `BLOSSOM_MEMORY_RERANKER_API_URL` / `BLOSSOM_MEMORY_RERANKER_API_KEY` |
| 参数 | `BLOSSOM_MEMORY_VECTOR_DIMENSION` / `BLOSSOM_MEMORY_RETRIEVAL_TOP_K` / `BLOSSOM_MEMORY_RERANK_TOP_K` / `BLOSSOM_MEMORY_SIMILARITY_THRESHOLD` / `BLOSSOM_MEMORY_MAX_ENTRIES` / `BLOSSOM_MEMORY_TTL_DAYS` / `BLOSSOM_MEMORY_DAILY_EXTRACT_LIMIT` |
