...
## 首次启动释放的 `.env` 模板（与 Web UI 同源）

项目根**没有 `.env` 时**首次启动会自动释放一份完整模板（`main.ensure_env_template()` →
`src/services/env_template.py`），它由 `config_schema.SCHEMA` 渲染：

- **170 项配置各自带中文说明**，并按与 Web UI 配置页**相同的分类顺序**分组（分类标签也复用同一份常量）；
- 必填项 / 密钥项 / 需重启项在模板里直接标注；
- **与 Web UI 配置页同一份单源** —— 改配置声明时模板与页面一起变，不会各写一份慢慢漂移；
- 已有 `.env` 不会被覆盖（只在该文件不存在时释放），因此升级不会冲掉你的配置。
