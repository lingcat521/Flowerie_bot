...
当前 **1140** 个测试（随版本增长）：并发安全、故障隔离、熔断、状态治理、Prompt/Sticker/MCP/Web UI、SSRF/注入回归与 Code Scanning 整改回归、MCP 额度/安全、配置持久化/校验、Web UI 注册/无 JS 面板、Persona 系统、群聊 Meme Knowledge、多条回复与 Native Reply Tool、任意语言插件（13 种语言黑盒端到端）等，详见 `tests/`。
...
tests/              # 1140 个测试（含 CI 资产/响应式回归/安全回归）
docs/               # 文档
```
