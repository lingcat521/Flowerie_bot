# 审计报告：Web UI 功能开关治理 + LivingMemory（qwq.txt Phase 1）

> 结论：架构已有良好基础（配置优先级链/Repository 接口/内存分层/无 JS 折叠先例/资源上限治理）。
> 本轮是**增量产品化**：6 个开关补全 + LivingMemory（Embedding/Vector/Reranker 新抽象，默认 OFF）
...
- **必须重启**：连接类（HTTP_API_BASE/WS 地址/API key 通道初始化）、资源初始化类（sticker 扫描、MCP client fail-fast、未来的 embedding client——**本轮 LivingMemory 开关须在初始化层门控**）。
...
| **LivingMemory（新增）** | 无 | **须实现**：max sets/TTL/每日提取上限/清理任务/索引（§25 全项） |
...
- 高级配置默认折叠；LivingMemory 未开启（或配置不完整）时 UI 显示状态徽标
