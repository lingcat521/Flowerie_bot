...
def test_milky_protocol_selects_milky_parser():
    ad = make_adapters(123, _Sender(), protocol="milky")
    assert ad.transport == "milky"
    assert isinstance(ad.parser, MilkyEventParser)


def test_onebot_default_unchanged():
    ad = make_adapters(123, _Sender())
    assert ad.transport == "onebot"
    assert isinstance(ad.parser, OneBotEventParser)


def test_milky_parser_parses_segments():
    ad = make_adapters(123, _Sender(), protocol="milky")
