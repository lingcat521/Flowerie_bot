...
#      环境里没有 @types/node 时，额外带上 SDK 自带的最小宿主类型声明（shims/node.d.ts）——
#      SDK 零 npm 依赖，不该为了编译它而要求装 @types/node。
set -e
EX_DIR="$(cd "$(dirname "$0")" && pwd)"
SDK_DIR="$EX_DIR/../../sdk/typescript"
BUILD_DIR="${FLOWERIE_TS_BUILD_DIR:-$(mktemp -d)}"
mkdir -p "$BUILD_DIR"
cp "$SDK_DIR/flowerie_sdk.ts" "$BUILD_DIR/flowerie_sdk.ts"
...
set -- "$BUILD_DIR/plugin_cjs.ts" "$BUILD_DIR/flowerie_sdk.ts"
if ! node -e 'require.resolve("@types/node/package.json")' >/dev/null 2>&1; then
  cp "$SDK_DIR/shims/node.d.ts" "$BUILD_DIR/node.d.ts"
  set -- "$@" "$BUILD_DIR/node.d.ts"
fi

if ! tsc "$@" --target es2022 --module node16 --moduleResolution node16 --outDir "$BUILD_DIR/js" >&2; then
  tsc "$@" --target es2022 --module commonjs --outDir "$BUILD_DIR/js" >&2
fi
